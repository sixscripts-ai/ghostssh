...
export class ApplicationKitService {
  async create(profile:CandidateProfile,job:RankedJob,provider?:ProviderName, apiKey?: string): Promise<ApplicationKit> {
    const raw=await withFallback(llm=>llm.generate({system:coverLetterPrompt.system,json:true,maxOutputTokens:1500,user:JSON.stringify({profile,job},null,2), apiKey}),provider);
